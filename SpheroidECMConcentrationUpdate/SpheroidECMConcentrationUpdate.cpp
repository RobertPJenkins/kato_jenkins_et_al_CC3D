...
#include <iomanip>
#include <sstream>
#include "SpheroidECMConcentrationUpdate.h"
...
SpheroidECMConcentrationUpdate::SpheroidECMConcentrationUpdate() :  cellFieldG(0),sim(0),potts(0) {}

SpheroidECMConcentrationUpdate::~SpheroidECMConcentrationUpdate() {
}


void SpheroidECMConcentrationUpdate::init(Simulator *simulator, CC3DXMLElement *_xmlData) {
	...
	boundaryPixelTrackerPlugin=(BoundaryPixelTrackerPlugin*)Simulator::pluginManager.get("BoundaryPixelTracker",&pluginAlreadyRegisteredFlag); //this will load VolumeTracker plugin if it is not already loaded
	if(!pluginAlreadyRegisteredFlag)
...
void SpheroidECMConcentrationUpdate::extraInit(Simulator *simulator){

}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void SpheroidECMConcentrationUpdate::start(){
...
void SpheroidECMConcentrationUpdate::step(const unsigned int currentStep){
	...
	if(currentStep==0){
		...
		for (int i=0; i<(N-1); i++){
 			int r=i + (rand() % (N-i));
			int temp = randomarray[i]; randomarray[i] = randomarray[r]; randomarray[r] = temp;
		}
		...
		for (int m=0;m<N;m++){
			...
			ECMconcfield[xx][yy][zz]=ECMconcfield[xx][yy][zz]-currkdeg;
			if(ECMconcfield[xx][yy][zz]<0.0){ECMconcfield[xx][yy][zz]=0.0;}
...
void SpheroidECMConcentrationUpdate::ECMOutput(){
	const int x=dimx;
	const int y=dimy;
	const int z=dimz;
	for (int i = 0; i< x; i++){
...
void SpheroidECMConcentrationUpdate::finish(){	
}


void SpheroidECMConcentrationUpdate::update(CC3DXMLElement *_xmlData, bool _fullInitFlag){
...
std::string SpheroidECMConcentrationUpdate::toString(){
   return "SpheroidECMConcentrationUpdate";
}


std::string SpheroidECMConcentrationUpdate::steerableName(){
   return toString();
}
